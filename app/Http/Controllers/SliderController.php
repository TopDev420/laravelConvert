...
class SliderController extends Controller
{
    ...
    public $listing_cols = ['id', 'title', 'image', 'content', 'description_second'];
    ...
    public function show($id)
    {
        if (Module::hasAccess("Slider_Images", "view")) {
            ...
            $frontpage = Slider_Image::find($id);
            if (isset($frontpage->id)) {
                $module = Module::get('Slider_Images');
                $module->row = $frontpage;

                return view('la.slider.show', [
                    'module' => $module,
    ...
    public function edit($id)
    {
        if (Module::hasAccess("Slider_Images", "edit")) {
            ...
            if (isset($frontpage->id)) {
                ...
                $module = Module::get('Slider_Images');
                // print_r($module);die('143');
                $module->row = $frontpage;

                return view('la.slider.edit', [
                    'module' => $module,
                    'view_col' => $this->view_col,
                ])->with('frontpages', $frontpage);
            } else {
