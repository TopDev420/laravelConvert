...
function checksvalue(){
		var  industry=state=city=joblevels=Jobfunctions=comp=countrval=Specialtyexclude='';
		var Specialtyexclude_count =state_count =city_count=joblevels_count=Jobfunctions_count=coun_contry=0;
    	...
    	$('.filter_result:checked').each(function(){
			...
			if ($(this).val() && typee == 'Specialty' && secrhtype=='Exculde' ) {
				...
				$('#Specialty_number').addClass('c-badge-exclude');
				Specialtyexclude = Specialtyexclude+'<li class="filter-save-data" new-data="'+incld_value+'" data-id="Specialty"><span class="text-exclude">'+typedyna+'</span>  '+incld_value+'<button type="button" class="removefiles fa fa-times "></button></li>';
				Specialtyexclude_count++;
			}
		...
		$('.filter_result:checked').each(function(){
			...
			if ($(this).val() && typee == 'Specialty' && secrhtype=='include' ) {
				Specialtyexclude = Specialtyexclude+'<li class="filter-save-data" new-data="'+incld_value+'" data-id="Specialty"><span class="text-success">'+typedyna+'</span>  '+incld_value+'<button type="button" class="removefiles fa fa-times "></button></li>';
				Specialtyexclude_count++;
			}
		...
		if(companies.length > 0 ) {
	        $.each(companies,function(index,value){
	            if(value== null){
	                ...
	                comp = comp+'<li class="filter-save-data loca-tion" new-data="'+value+'" data-id="company" > <span class="'+dynacls+'">'+value.type+'</span> '+value.value+' <button type="button" class="removefiles fa fa-times loca-tion" ></button></li>';
		...
		if(Specialtyexclude != ''){
			$('#Specialty_number').show();
		 	$('.selected_value .'+ 'Specialty').html('');
		 	$('.selected_value .'+ 'Specialty').append(Specialtyexclude); 
		 	$('.industry_count').html("There is "+ Specialtyexclude_count +" Selected item"); 
			$('#Specialty_number').html(Specialtyexclude_count); 
		}
		if(coun_contry>0){
			$('#remove-country').show();
...
function validation(types){
	    ...
	    $('.filter_result:checked').each(function(){
	        ...
	        if (/*type =='city' &&*/ $(this).val() && typee == 'Specialty' && secrhtype=='include') {
	            // console.log('1302');
	            var datas = '';
	            filter = filter+'<li class="filter-save-data" new-data="'+incld_value+'" data-id="Specialty">'+'Specialty: '+incld_value+'<button type="button" class="removefiles fa fa-times "></button></li>';
	        }   

	        if (type =='Specialty' && $(this).val() && typee == 'Specialty' && secrhtype=='include') {
	            var datas = '';
	            filter1 = filter1+'<li class="filter-save-data" new-data="'+incld_value+'" data-id="Specialty"><span class="text-success">include</span>  '+incld_value+'<button type="button" class="tag-remove removefiles"></li>';
	        }  
	    });
	    if(type=='country'){
...
function validationexclude(types){
	    ...
	    $('.filter_result:checked').each(function(){
	        ...
	        if (/*type =='city' &&*/ $(this).val() && typee == 'Specialty' && secrhtype=='Exculde') {
	            // console.log('1302');
	            var datas = '';
	            filter = filter+'<li class="filter-save-data" new-data="'+incld_value+'" data-id="Specialty">'+'Specialty: '+incld_value+'<button type="button" class="removefiles fa fa-times "></button></li>';
	        }   

	        if (type =='Specialty' && $(this).val() && typee == 'Specialty' && secrhtype=='Exculde') {
	        	var opendivname = $('#'+typee).attr('data-box');
                if($('div#'+typee+' span.c-badge-success').length>0){
                    $('#'+typee+'_number').removeClass('c-badge-success');
                }
	            ...
	            var datas = '';
	            filter1 = filter1+'<li class="filter-save-data" new-data="'+incld_value+'" data-id="Specialty"><span class="text-exclude">exclude</span>  '+incld_value+'<button type="button" class="tag-remove removefiles"></li>';
	        }  
	    });
	    if(type=='country'){
	        $('#div_country   ul.c-choice-tree li  input[type=checkbox]:checked').each(function(){
