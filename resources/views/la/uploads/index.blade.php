...
$(function () {
    ...
    $("body").on("click", "ul.files_container .fm_file_sel", function() {
        ...
        $(".file-info-form input[name=url]").val(bsurl+'/storage/uploads/'+upload.name);
        $(".file-info-form input[name=caption]").val(upload.caption);
        $("#EditFileModal #downFileBtn").attr("href", bsurl+'/storage/uploads/'+upload.name+"?download");
        ...
        if($.inArray(upload.extension, ["jpg", "jpeg", "png", "gif", "bmp"]) > -1) {
            $("#EditFileModal .fileObject").append('<img src="'+bsurl+'/storage/uploads/'+upload.name+'">');
            $("#EditFileModal .fileObject").css("padding", "15px 0px");
        } else {
            switch (upload.extension) {
                case "pdf":
                    // TODO: Object PDF
                    $("#EditFileModal .fileObject").append('<object width="100%" height="325" data="'+bsurl+'/storage/uploads/'+upload.name+'"></object>');
...
function formatFile(upload) {
    var image = '';
    if($.inArray(upload.extension, ["jpg", "jpeg", "png", "gif", "bmp"]) > -1) {
        image = '<img src="'+bsurl+'/storage/uploads/'+upload.name+'?s=130">';
    } else {
        switch (upload.extension) {
            case "pdf":
